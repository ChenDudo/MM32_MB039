...
#if PRINTPKT_SUPPORT
static int eap_printpkt(const u_char* inp, int inlen, void (*)(void* arg, const char* fmt, ...), void* arg);
...
static const u_char wkmodulus[] = {
    ,
    ...
    switch (pcb->eap.es_server.ea_state) {
        ...
        case eapIdentify:
            ...
            if (pcb->eap.es_server.ea_peerlen > SRP_PSEUDO_LEN &&
                ...
                BZERO(&bs, sizeof(bs));
                plen  = b64dec(&bs, pcb->eap.es_server.ea_peer + SRP_PSEUDO_LEN, pcb->eap.es_server.ea_peerlen - SRP_PSEUDO_LEN,
                              secbuf);
                toffs = 0;
                for (i = 0; i < 5; i++) {
                    pncrypt_setkey(toffs);
                    toffs -= 86400;
                    /* FIXME: if we want to do SRP, we need to find a way to pass the PolarSSL des_context instead of using static
                     * memory */
                ...
                if (plen % 8 == 0 && i < 5) {
                    ...
                    sp = secbuf + 8;
                    while (plen > 0) {
                        /* FIXME: if we want to do SRP, we need to find a way to pass the PolarSSL des_context instead of using
                         * static memory */
                    ...
                    ppp_dbglog("decoded pseudonym to \"%.*q\"", pcb->eap.es_server.ea_peerlen, pcb->eap.es_server.ea_peer);
                }
                else {
        ...
        default: pcb->eap.es_server.ea_state = eapBadAuth; break;
    }
...
static void eap_send_request(ppp_pcb* pcb)
{
    ...
    if (pcb->settings.eap_max_transmits > 0 && pcb->eap.es_server.ea_requests >= pcb->settings.eap_max_transmits) {
...
static void eap_lowerdown(ppp_pcb* pcb)
{
    ...
    if (eap_server_active(pcb)) {
        ...
        if ((pcb->eap.es_server.ea_state == eapOpen || pcb->eap.es_server.ea_state == eapSRP4) && pcb->eap.es_rechallenge > 0) {
...
static void eap_request(ppp_pcb* pcb, u_char* inp, int id, int len)
{
    ...
    if (pcb->settings.eap_allow_req != 0 && pcb->eap.es_client.ea_requests > pcb->settings.eap_allow_req) {
    ...
    switch (typenum) {
        case EAPT_IDENTITY:
            ...
            if (pcb->eap.es_usepseudo &&
                (pcb->eap.es_usedpseudo == 0 || (pcb->eap.es_usedpseudo == 1 && id == pcb->eap.es_client.ea_id))) {
        ...
        case EAPT_MD5CHAP:
            ...
            if (pcb->settings.explicit_remote || (pcb->settings.remote_name[0] != '\0' && vallen == len))
            ...
            switch (vallen) {
                ...
                case EAPSRP_SKEY:
                    ...
                    if (pcb->eap.es_client.ea_skey != NULL) {
                        ...
                        if (get_srp_secret(pcb->eap.es_unit, pcb->eap.es_client.ea_name, pcb->eap.es_client.ea_peer, secret, 0) ==
                            0) {
                            /*
...
static void eap_response(ppp_pcb* pcb, u_char* inp, int id, int len)
{
    ...
    switch (typenum) {
        ...
        case EAPT_NOTIFICATION: ppp_dbglog("EAP unexpected Notification; response discarded"); break;
            ...
            switch (vallen) {
                ...
                default:
                    ...
                    switch (pcb->eap.es_server.ea_state) {
                        case eapSRP1:
                        case eapSRP2:
                        case eapSRP3: pcb->eap.es_server.ea_state = eapMD5Chall; break;
                        case eapMD5Chall:
                        case eapSRP4:
                            pcb->eap.es_server.ea_state = eapIdentify;
                            eap_figure_next_state(pcb, 0);
...
static void eap_success(ppp_pcb* pcb, u_char* inp, int id, int len)
{
    ...
    if (pcb->eap.es_client.ea_state != eapOpen && !eap_client_active(pcb)) {
        ppp_dbglog("EAP unexpected success message in state %s (%d)", eap_state_name(pcb->eap.es_client.ea_state),
...
static void eap_failure(ppp_pcb* pcb, u_char* inp, int id, int len)
{
    ...
    if (!eap_client_active(pcb)) {
        ppp_dbglog("EAP unexpected failure message in state %s (%d)", eap_state_name(pcb->eap.es_client.ea_state),
                   pcb->eap.es_client.ea_state);
    }
...
static void eap_input(ppp_pcb* pcb, u_char* inp, int inlen)
{
    ...
    switch (code) {
        case EAP_REQUEST: eap_request(pcb, inp, id, len); break;

#if PPP_SERVER
        case EAP_RESPONSE: eap_response(pcb, inp, id, len); break;
#endif /* PPP_SERVER */

        case EAP_SUCCESS: eap_success(pcb, inp, id, len); break;

        case EAP_FAILURE: eap_failure(pcb, inp, id, len); break;
...
static const char* const eap_typenames[] = {
    "Identity", "Notification", "Nak", "MD5-Challenge", "OTP",          "Generic-Token", NULL,    NULL,    "RSA", "DSS",
    "KEA",      "KEA-Validate", "TLS", "Defender",      "Windows 2000", "Arcot",         "Cisco", "Nokia", "SRP"};
