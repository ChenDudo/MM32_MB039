...
/* OUT_BUF_SIZE is the size of the output buffer used during decompression.
   OUT_BUF_SIZE must be a power of 2 >= TINFL_LZ_DICT_SIZE (because the low-level decompressor not only writes, but reads from the
   output buffer as it decompresses) */
...
{
    printf(" Usage: htmlgen [targetdir] [-s] [-e] [-11] [-nossi] [-ssi:<filename>] [-c] [-f:<filename>] [-m] [-svr:<name>] "
           "[-x:<ext_list>] [-xc:<ext_list>" USAGE_ARG_DEFLATE NEWLINE NEWLINE);
...
int main(int argc, char* argv[])
{
    ...
    printf("HTTP %sheader will %s statically included." NEWLINE, (includeHttpHeader ? (useHttp11 ? "1.1 " : "1.0 ") : ""),
     ...
     */
    fprintf(data_file, "#ifndef FS_FILE_FLAGS_HEADER_PERSISTENT" NEWLINE "#define FS_FILE_FLAGS_HEADER_PERSISTENT 0" NEWLINE
                       "#endif" NEWLINE);
    ...
    if (deflateNonSsiFiles) {
        printf("(Deflated total byte reduction: %d bytes -> %d bytes (%.02f%%)" NEWLINE, (int)overallDataBytes,
               (int)deflatedBytesReduced, (float)((deflatedBytesReduced * 100.0) / overallDataBytes));
    }
#endif
    printf(NEWLINE);
    ...
    if (deflateNonSsiFiles) {
        if (can_be_compressed) {
            if (fsize < OUT_BUF_SIZE) {
                ...
                /* create tdefl() compatible flags (we have to compose the low-level flags ourselves, or use
                 * tdefl_create_comp_flags_from_zip_params() but that means MINIZ_NO_ZLIB_APIS can't be defined). */
                mz_uint comp_flags =
                    s_tdefl_num_probes[MZ_MIN(10, deflate_level)] | ((deflate_level <= 3) ? TDEFL_GREEDY_PARSING_FLAG : 0);
                if (!deflate_level) {
                ...
                if (out_bytes < fsize) {
                    ...
                    memcpy(ret_buf, s_outbuf, out_bytes);
                    {
                        ...
                        dec_status = tinfl_decompress(&inflator, (const mz_uint8*)ret_buf, &dec_in_bytes, s_checkbuf,
                                                      (mz_uint8*)next_out, &dec_out_bytes, 0);
                    ...
                    buf        = ret_buf;
                    *file_size = out_bytes;
                    printf(" - deflate: %d bytes -> %d bytes (%.02f%%)" NEWLINE, (int)fsize, (int)out_bytes,
...
static int
write_checksums(FILE* struct_file, const char* varname, u16_t hdr_len, u16_t hdr_chksum, const u8_t* file_data, size_t file_size)
{
...
static int ext_in_list(const char* filename, const char* ext_list)
{
    ...
    while (*ext != '\0') {
        ...
        ext_size = comma - ext;
        if ((filename[filename_size - ext_size - 1] == '.') && !strncmp(&filename[filename_size - ext_size], ext, ext_size)) {
            found = 1;
            break;
        }
        ext = comma + 1;
...
int process_file(FILE* data_file, FILE* struct_file, const char* filename)
{
    ...
    fprintf(data_file, "static const " PAYLOAD_ALIGN_TYPE " dummy_align_%s = %d;" NEWLINE, varname,
