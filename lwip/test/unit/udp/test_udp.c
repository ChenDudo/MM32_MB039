...
static void default_netif_add(void)
{
    ...
    n = netif_add(&test_netif1, &test_ipaddr1, &test_netmask1, &test_gw1, NULL, default_netif_init, NULL);
    ...
    n = netif_add(&test_netif2, &test_ipaddr2, &test_netmask2, &test_gw2, NULL, default_netif_init, NULL);
...
static void test_recv(void* arg, struct udp_pcb* pcb, struct pbuf* p, const ip_addr_t* addr, u16_t port)
{
...
{
    testfunc tests[] = {TESTFUNC(test_udp_new_remove), TESTFUNC(test_udp_broadcast_rx_with_2_netifs), TESTFUNC(test_udp_bind)};
