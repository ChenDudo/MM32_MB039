...
START_TEST(test_tcp_listen_passive_open)
{
    ...
    p = tcp_create_segment(&src_addr, &lpcb->local_ip, 12345, lpcb->local_port, NULL, 0, 12345, 54321, TCP_SYN);
    EXPECT(p != NULL);
    if (p != NULL) {
...
START_TEST(test_tcp_malformed_header)
{
    ...
    chksum = ip_chksum_pseudo(p, IP_PROTO_TCP, p->tot_len, &test_remote_ip, &test_local_ip);
...
static void test_tcp_rto_timeout_syn_sent_impl(int link_down)
{
    ...
    const u16_t                tcp_syn_opts_len =
        LWIP_TCP_OPT_LENGTH(TF_SEG_OPTS_MSS | TF_SEG_OPTS_WND_SCALE | TF_SEG_OPTS_SACK_PERM | TF_SEG_OPTS_TS);
