...
static int tcp_oos_count(struct tcp_pcb* pcb)
{
    ...
    (PBUF_POOL_BUFSIZE >= (TCP_MSS + PBUF_LINK_ENCAPSULATION_HLEN + PBUF_LINK_HLEN + PBUF_IP_HLEN + PBUF_TRANSPORT_HLEN))
...
START_TEST(test_tcp_recv_ooseq_FIN_INSEQ)
{
    ...
    if ((pinseq != NULL) && (p_1_2 != NULL) && (p_4_8 != NULL) && (p_3_11 != NULL) && (p_2_12 != NULL) && (p_15_1 != NULL) &&
        (p_15_1a != NULL) && (pinseqFIN != NULL)) {
        /* pass the segment to tcp_input */
...
START_TEST(test_tcp_recv_ooseq_overrun_rxwin)
{
    ...
    for (i = TCP_MSS, k = 0; i < TCP_WND; i += TCP_MSS, k++) {
        ...
        struct pbuf* p = tcp_create_rx_segment(pcb, &data_full_wnd[TCP_MSS * (k + 1)], TCP_MSS, TCP_MSS * (k + 1), 0, TCP_ACK);
        EXPECT_RET(p != NULL);
        /* pass the segment to tcp_input */
...
START_TEST(test_tcp_recv_ooseq_max_bytes)
{
    ...
    for (k = 1, i = 1; k < TCP_OOSEQ_MAX_BYTES; k += TCP_MSS, i++) {
        int          count;
        struct pbuf* p = tcp_create_rx_segment(pcb, &data_full_wnd[k], TCP_MSS, k, 0, TCP_ACK);
        EXPECT_RET(p != NULL);
    ...
    tcp_abort(pcb);
    EXPECT(MEMP_STATS_GET(used, MEMP_TCP_PCB) == 0);
#endif /* TCP_OOSEQ_MAX_BYTES && (TCP_OOSEQ_MAX_BYTES < (TCP_WND + 1)) && (PBUF_POOL_BUFSIZE >= (TCP_MSS +                       \
...
START_TEST(test_tcp_recv_ooseq_max_pbufs)
{
    ...
    for (i = 1; i <= TCP_OOSEQ_MAX_PBUFS; i++) {
        int          count;
        struct pbuf* p = tcp_create_rx_segment(pcb, &data_full_wnd[i], 1, i, 0, TCP_ACK);
        EXPECT_RET(p != NULL);
        EXPECT_RET(p->next == NULL);
        /* pass the segment to tcp_input */
        test_tcp_input(p, &netif);
    ...
    tcp_abort(pcb);
    EXPECT(MEMP_STATS_GET(used, MEMP_TCP_PCB) == 0);
#endif /* TCP_OOSEQ_MAX_PBUFS && (TCP_OOSEQ_MAX_BYTES < (TCP_WND + 1)) && (PBUF_POOL_BUFSIZE >= (TCP_MSS +                       \
          PBUF_LINK_ENCAPSULATION_HLEN + PBUF_LINK_HLEN + PBUF_IP_HLEN + PBUF_TRANSPORT_HLEN)) */
    LWIP_UNUSED_ARG(_i);
}
END_TEST

static void check_rx_counters(struct tcp_pcb*           pcb,
                              struct test_tcp_counters* counters,
...
{
    testfunc tests[] = {TESTFUNC(test_tcp_recv_ooseq_FIN_OOSEQ),     TESTFUNC(test_tcp_recv_ooseq_FIN_INSEQ),
                        TESTFUNC(test_tcp_recv_ooseq_overrun_rxwin), TESTFUNC(test_tcp_recv_ooseq_overrun_rxwin_edge),
                        TESTFUNC(test_tcp_recv_ooseq_max_bytes),     TESTFUNC(test_tcp_recv_ooseq_max_pbufs),
                        TESTFUNC(test_tcp_recv_ooseq_double_FIN_0),  TESTFUNC(test_tcp_recv_ooseq_double_FIN_1),
                        TESTFUNC(test_tcp_recv_ooseq_double_FIN_2),  TESTFUNC(test_tcp_recv_ooseq_double_FIN_3),
                        TESTFUNC(test_tcp_recv_ooseq_double_FIN_4),  TESTFUNC(test_tcp_recv_ooseq_double_FIN_5),
                        TESTFUNC(test_tcp_recv_ooseq_double_FIN_6),  TESTFUNC(test_tcp_recv_ooseq_double_FIN_7),
                        TESTFUNC(test_tcp_recv_ooseq_double_FIN_8),  TESTFUNC(test_tcp_recv_ooseq_double_FIN_9),
                        TESTFUNC(test_tcp_recv_ooseq_double_FIN_10), TESTFUNC(test_tcp_recv_ooseq_double_FIN_11),
                        TESTFUNC(test_tcp_recv_ooseq_double_FIN_12), TESTFUNC(test_tcp_recv_ooseq_double_FIN_13),
                        TESTFUNC(test_tcp_recv_ooseq_double_FIN_14), TESTFUNC(test_tcp_recv_ooseq_double_FIN_15)};
