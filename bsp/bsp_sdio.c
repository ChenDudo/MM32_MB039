...
SD_Error SD_PowerON(void)
{
    ...
    SDIO_Send_Cmd(SDIO_SEND_IF_COND, SDIO_Response_Short,
                  SD_CHECK_PATTERN);                // Send CMD8, short response, check SD card interface features.
...
SD_Error SD_InitializeCards(void)
{
    ...
    if (SDIO_SECURE_DIGITAL_IO_CARD != CardType) {                  // The SECURE_DIGITAL_IO_CARD
        ...
        CID_Tab[3] = SDIO->CMD_BUF2 << 24 | SDIO->CMD_BUF1 << 16 | SDIO->CMD_BUF0 << 8 |
                     SDIO->CMD_BUF0;  // The low 8 bits are CRC and reserved bits
    ...
    if ((SDIO_STD_CAPACITY_SD_CARD_V1_1 == CardType) || (SDIO_STD_CAPACITY_SD_CARD_V2_0 == CardType) ||
        (SDIO_SECURE_DIGITAL_IO_COMBO_CARD == CardType) || (SDIO_HIGH_CAPACITY_SD_CARD == CardType)) {  // Judgment card type
...
SD_Error SD_Init(void)
{
    ...
    if ((errorstatus != SD_OK)) {
        if ((SDIO_MULTIMEDIA_CARD == CardType)) {
            /* V1.1/V2.0 card with the maximum setting of 48/4=12Mhz; For other cards such as SDHC, the maximum setting is
             * 48/2=24Mhz */
            clkdiv =
                (SDCardInfo.CardType == SDIO_STD_CAPACITY_SD_CARD_V1_1 || SDCardInfo.CardType == SDIO_STD_CAPACITY_SD_CARD_V2_0)
                    ? 0
                    : 1;
        ...
        clkdiv = (SDCardInfo.CardType == SDIO_STD_CAPACITY_SD_CARD_V1_1 || SDCardInfo.CardType == SDIO_STD_CAPACITY_SD_CARD_V2_0)
                     ? 0
                     : 1;
        targetFreq = (clkdiv != 0) ? 24000000 : 12000000;
        RCC_GetClocksFreq(&bclk);
...
SD_Error SD_GetCardInfo(SD_CardInfo* cardinfo)
{
    ...
    cardinfo->SD_csd.SysSpecVersion =
        (tmp & 0x3C) >>
        2;  // This part of the 2.0 protocol has not been defined (for reservation) and should be defined by subsequent protocols
...
u8 SD_ReadDisk(u8* buf, u32 sector, u32 cnt)
{
    ...
    if ((u32)buf % 4 != 0) {
        ...
        if (cnt == 1)
            sta = SD_ReadBlocks(buf, lsector, 512, 1);
        else
            sta = SD_ReadMulBlocks(buf, lsector, 512, cnt);
    }
    return sta;
}
...
u8 SD_WriteDisk(u8* buf, u32 sector, u32 cnt)
{
    ...
    if ((u32)buf % 4 != 0) {
        ...
        if (cnt == 1)
            sta = SD_WriteBlocks(buf, lsector, 512);
